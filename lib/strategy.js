 ...
 *
 * Options:
 *   - `clientID`      your Stripe application's client ID
 *   - `clientSecret`  your Stripe application's App Secret
 *   - `callbackURL`   URL to which Stripe will redirect the user after granting authorization
 *
 * Examples:
...
function Strategy(options, verify) {
  ...
  options.authorizationURL =
    options.authorizationURL || 'https://connect.stripe.com/oauth/authorize';
  options.tokenURL =
    options.tokenURL || 'https://connect.stripe.com/oauth/token';
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    const state = options.state || req.query.state;
    if (state) {
      params.state = state;
    }
